...
void information_start(list_t* list) {
	...
	while (choice) {
		...
		case 8: save_list_to_binary_file(list); break;
		case 9: load_list_from_binary_file(list); break;
		case 10: find_in_file_by_author(); break;
		case 11: save_info_to_the_end_of_file(list); break;
		}
	}
}
...
void change_info(list_t* list) {
	...
	if (i < 0 || i > list->size - 1) {
		...
		wprintf(L"\n[%d] Element:", i);
		wchar_t buff[STR_BUFFER_SIZE];
		swprintf(buff, STR_BUFFER_SIZE, L"\nДержатель информации: %s ->", obj->keeper);
...
{
	wchar_t* keeper = prep_str(STR_BUFFER_SIZE);
	size_t keeper_len;

	printf("\nДержатель информации: ");
	_cgetws_s(keeper, STR_BUFFER_SIZE, &keeper_len);

	float size = 0;
	printf("\nРазмер информации: ");
	scanf_s("%f", &size);

	wchar_t* name = prep_str(STR_BUFFER_SIZE);
	size_t name_len;
	printf("\nНазвание: ");
	_cgetws_s(name, STR_BUFFER_SIZE, &name_len);

	wchar_t* author = prep_str(STR_BUFFER_SIZE);
	size_t author_len;
	printf("\nАвтор: ");
	_cgetws_s(author, STR_BUFFER_SIZE, &author_len);
